...
# as mean per chromosome
rearrangements_len %>% filter(arr == "rearrangement") %>% pull(pct_len) %>% mean()

# as total length of genome alignments
rearrangements_len %>% group_by(arr) %>% summarize(total = sum(total_len)) %>% mutate(pct_len = total/sum(total) * 100) 
